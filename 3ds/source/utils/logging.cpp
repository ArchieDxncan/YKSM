...
namespace
{
    ...
    {
        return std::format(
            "YKSM v{:d}.{:d}.{:d}-{:s}", VERSION_MAJOR, VERSION_MINOR, VERSION_MICRO, GIT_REV);
    }
...
#if defined(__3DS__)
    logFilePath = std::string("sdmc:/3ds/YKSM/logs/yksm_") + dateBuf + ".log";
#elif defined(__SWITCH__)
    logFilePath = std::string("/switch/YKSM/logs/yksm_") + dateBuf + ".log";
#else
    logFilePath = std::string("yksm_") + dateBuf + ".log";
#endif
